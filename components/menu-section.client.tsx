...
export function MenuSectionClient({ menu, dishes, locale }: { 
  ...
  const categoryOrderMap = useMemo(() => {
    ...
    for (const d of dishes ?? []) {
      const name = d.category_dish.name
  ...
  const categories = useMemo(() => {
    const uniq = Array.from(new Set(dishes?.map((d: Dish) => d.category_dish.name) ?? []))
  ...
  const filteredDishes = useMemo(() => {
    const list = activeCategory === "Tất cả"
      ? (dishes ?? [])
      : (dishes?.filter((d: Dish) => d.category_dish.name === activeCategory) ?? [])
